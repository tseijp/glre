...
describe('Type Inference', () => {
        describe('Function return types', () => {
                it('preserves input type', () => {
                        const x = sin(float(1.57))
                        const y = normalize(vec3(1, 2, 3))
                        expect(inferAndCode(x).type).toBe('float')
                        expect(inferAndCode(y).type).toBe('vec3')
                })

                it('returns scalar', () => {
                        const x = dot(vec3(1, 0, 0), vec3(0, 1, 0))
                        expect(inferAndCode(x).type).toBe('float')
                })

                it('highest type promotion', () => {
                        const x = mix(vec3(1, 0, 0), vec3(0, 1, 0), float(0.5))
        ...
        describe('Complex expressions', () => {
                it('nested operations', () => {
                        const x = mix(vec3(1, 0, 0), vec3(0, 1, 0), float(0.5)).add(vec3(0, 0, 1))
                        const y = sin(vec2(1, 2).x).mul(cos(vec2(1, 2).y))
                        expect(inferAndCode(x).type).toBe('vec3')
        ...
        describe('Swizzle inference', () => {
                it('component count', () => {
                        const x = vec3(1, 2, 3).xy
                        const y = vec2(1, 2).xxxx
                        const z = vec3(1, 2, 3).z
