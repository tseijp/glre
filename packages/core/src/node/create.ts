...
export const create = <T extends C>(type: NodeTypes, props?: NodeProps | null, ...children: Y[]) => {
        ...
        const get = (_: unknown, key: string | Symbol) => {
                ...
                if (key === 'varying') return varying.bind(null, x)
                if (key === 'uniform') return uniform.bind(null, x)
                if (key === 'storage') return storage.bind(null, x)
                if (key === 'instance') return instance.bind(null, x)
                if (key === 'constant') return constant.bind(null, x)
                if (key === 'attribute') return attribute.bind(null, x)
...
export const instancedArray = <T extends C>(x: Y<T>, id = getId()) => create<T>('instanceArray', { id }, x)
export const attributeArray = <T extends C>(x: Y<T>, id = getId()) => create<T>('attributeArray', { id }, x)
export const uniformArray = <T extends C>(x: Y<T>, id = getId(), count?: number) => create<T>('uniformArray', { id, args: count ? [count] : [] }, x)
