...
export const texture = (x: X, y: X, z?: X) => f<'vec4'>(['vec4'], 'texture', x, y, z)
export const cubeTexture = (x: X, y: X, z?: X) => f<'vec4'>(['vec4'], 'cubeTexture', x, y, z)
export const textureSize = (x: X, y?: X) => f<'vec4'>(['vec4'], 'textureSize', x, y)

// Component-wise functions - preserve input type (T -> T)
export const abs = <T extends C>(x: X<T>) => f<T>([x], 'abs', x)
export const sign = <T extends C>(x: X<T>) => f<T>([x], 'sign', x)
export const floor = <T extends C>(x: X<T>) => f<T>([x], 'floor', x)
export const ceil = <T extends C>(x: X<T>) => f<T>([x], 'ceil', x)
export const round = <T extends C>(x: X<T>) => f<T>([x], 'round', x)
export const fract = <T extends C>(x: X<T>) => f<T>([x], 'fract', x)
export const trunc = <T extends C>(x: X<T>) => f<T>([x], 'trunc', x)
export const sin = <T extends C>(x: X<T>) => f<T>([x], 'sin', x)
export const cos = <T extends C>(x: X<T>) => f<T>([x], 'cos', x)
export const tan = <T extends C>(x: X<T>) => f<T>([x], 'tan', x)
export const asin = <T extends C>(x: X<T>) => f<T>([x], 'asin', x)
export const acos = <T extends C>(x: X<T>) => f<T>([x], 'acos', x)
export const atan = <T extends C>(x: X<T>) => f<T>([x], 'atan', x)
export const sinh = <T extends C>(x: X<T>) => f<T>([x], 'sinh', x)
export const cosh = <T extends C>(x: X<T>) => f<T>([x], 'cosh', x)
export const tanh = <T extends C>(x: X<T>) => f<T>([x], 'tanh', x)
export const asinh = <T extends C>(x: X<T>) => f<T>([x], 'asinh', x)
export const acosh = <T extends C>(x: X<T>) => f<T>([x], 'acosh', x)
export const atanh = <T extends C>(x: X<T>) => f<T>([x], 'atanh', x)
export const exp = <T extends C>(x: X<T>) => f<T>([x], 'exp', x)
export const exp2 = <T extends C>(x: X<T>) => f<T>([x], 'exp2', x)
export const log = <T extends C>(x: X<T>) => f<T>([x], 'log', x)
export const log2 = <T extends C>(x: X<T>) => f<T>([x], 'log2', x)
export const sqrt = <T extends C>(x: X<T>) => f<T>([x], 'sqrt', x)
export const inverseSqrt = <T extends C>(x: X<T>) => f<T>([x], 'inverseSqrt', x)
export const normalize = <T extends C>(x: X<T>) => f<T>([x], 'normalize', x)
export const oneMinus = <T extends C>(x: X<T>) => f<T>([x], 'oneMinus', x)
export const saturate = <T extends C>(x: X<T>) => f<T>([x], 'saturate', x)
export const negate = <T extends C>(x: X<T>) => f<T>([x], 'negate', x)
export const reciprocal = <T extends C>(x: X<T>) => f<T>([x], 'reciprocal', x)
export const dFdx = <T extends C>(x: X<T>) => f<T>([x], 'dFdx', x)
export const dFdy = <T extends C>(x: X<T>) => f<T>([x], 'dFdy', x)
export const fwidth = <T extends C>(x: X<T>) => f<T>([x], 'fwidth', x)
export const degrees = <T extends C>(x: X<T>) => f<T>([x], 'degrees', x)
export const radians = <T extends C>(x: X<T>) => f<T>([x], 'radians', x)

// Functions that always return float regardless of input
export const length = (x: X) => f<'float'>(['float'], 'length', x)
export const lengthSq = (x: X) => f<'float'>(['float'], 'lengthSq', x)
export const distance = (x: X, y: X) => f<'float'>(['float'], 'distance', x, y)
export const dot = (x: X, y: X) => f<'float'>(['float'], 'dot', x, y)

// Functions that always return bool
export const all = <T extends C>(x: X<T>) => f<'bool'>(['bool'], 'all', x)
export const any = <T extends C>(x: X<T>) => f<'bool'>(['bool'], 'any', x)

// Functions that always return vec3 (cross product only works with vec3)
export const cross = (x: X<'vec3'>, y: X<'vec3'>) => f<'vec3'>(['vec3'], 'cross', x, y)

// Functions where first argument determines return type
export const reflect = <T extends C>(I: X<T>, N: X) => f<T>([I], 'reflect', I, N)
export const refract = <T extends C>(I: X<T>, N: X, eta: X) => f<T>([I], 'refract', I, N, eta)

// Functions with highest priority type among arguments (using first arg for simplicity)
export const min = <T extends C>(x: X<T>, y: X) => f<T>([x], 'min', x, y)
export const max = <T extends C>(x: X<T>, y: X) => f<T>([x], 'max', x, y)
export const mix = <T extends C>(x: X<T>, y: X, a: X) => f<T>([x], 'mix', x, y, a)
export const clamp = <T extends C>(x: X<T>, minVal: X, maxVal: X) => f<T>([x], 'clamp', x, minVal, maxVal)
export const step = <T extends C>(edge: X, x: X<T>) => f<T>([x], 'step', edge, x)
export const smoothstep = <T extends C>(e0: X, e1: X, x: X<T>) => f<T>([x], 'smoothstep', e0, e1, x)

// Two-argument functions with highest priority type
export const atan2 = <T extends C>(y: X<T>, x: X) => f<T>([y], 'atan2', y, x)
export const pow = <T extends C>(x: X<T>, y: X) => f<T>([x], 'pow', x, y)

// Component-wise power functions
export const pow2 = <T extends C>(x: X<T>) => f<T>([x], 'pow2', x)
export const pow3 = <T extends C>(x: X<T>) => f<T>([x], 'pow3', x)
export const pow4 = <T extends C>(x: X<T>) => f<T>([x], 'pow4', x)

// Utility functions
export const bitcast = <T extends C>(x: X<T>, y: X) => f<T>([x], 'bitcast', x, y)
export const cbrt = <T extends C>(x: X<T>) => f<T>([x], 'cbrt', x)
export const difference = <T extends C>(x: X<T>, y: X) => f<T>([x], 'difference', x, y)
export const equals = (x: X, y: X) => f<'bool'>(['bool'], 'equals', x, y)
export const faceforward = <T extends C>(N: X<T>, I: X, Nref: X) => f<T>([N], 'faceforward', N, I, Nref)
export const transformDirection = <T extends C>(dir: X<T>, matrix: X) => f<T>([dir], 'transformDirection', dir, matrix)
