...
export const distEuclidean = Fn(([a, b]: [X, X]): Float => {
        return distance(a, b)
}).setLayout({
        name: 'distEuclidean',
        type: 'float',
...
export const distManhattan4 = Fn(([a, b]: [Vec4, Vec4]): Float => {
        return abs(a.x.sub(b.x)).add(abs(a.y.sub(b.y))).add(abs(a.z.sub(b.z))).add(abs(a.w.sub(b.w)))
}).setLayout({
        name: 'distManhattan',
        type: 'float',
        inputs: [
...
export const distMinkowski = Fn(([a, b]: [Vec2, Vec2]): Float => {
        ...
        return pow(
                pow(abs(a.x.sub(b.x)), p).add(pow(abs(a.y.sub(b.y)), p)),
                1.0 / p
        )
}).setLayout({
        name: 'distMinkowski',
...
export const distMinkowski3 = Fn(([a, b]: [Vec3, Vec3]): Float => {
        const p = DIST_MINKOWSKI_P
        return pow(
                pow(abs(a.x.sub(b.x)), p).add(pow(abs(a.y.sub(b.y)), p)).add(pow(abs(a.z.sub(b.z)), p)),
                1.0 / p
        )
}).setLayout({
        name: 'distMinkowski',
...
export const distMinkowski4 = Fn(([a, b]: [Vec4, Vec4]): Float => {
        const p = DIST_MINKOWSKI_P
        return pow(
                pow(abs(a.x.sub(b.x)), p).add(pow(abs(a.y.sub(b.y)), p)).add(pow(abs(a.z.sub(b.z)), p)).add(pow(abs(a.w.sub(b.w)), p)),
                1.0 / p
        )
}).setLayout({
        name: 'distMinkowski',
