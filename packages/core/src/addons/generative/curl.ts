import { Fn, Vec2, Vec3, Vec4, vec2, vec3, vec4, float, normalize } from '../../node'
...
export const curlVec2 = Fn(([p]: [Vec2]): Vec2 => {
        ...
        const divisor = float(1).div(float(2).mul(e)).toVar('divisor')
        return normalize(vec2(x, y).mul(divisor))
}).setLayout({
        name: 'curlVec2',
        type: 'vec2',
