...
```jsx live
function Canvas() {
        const ellipseRadius = uniform(0.5, 'ellipseRadius')
        const ellipseXCoord = uniform(1.0, 'ellipseXCoord')
        const ellipseYCoord = uniform(1.0, 'ellipseYCoord')
        const ellipseStroke = uniform(0.1, 'ellipseStroke')
