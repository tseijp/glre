...
- $s = 2.0$ (frequency scaling factor, lacunarity)
...
| Scale Scalar      | 2.0     | Frequency increase per octave  |
...
const fragment = () => {
  const p = uv.mul(3)
  const base = fbmVec2(p)
  const warped = fbmVec2(p.add(vec2(base.mul(0.4))))
  const detail = base.mul(0.6).add(warped.mul(0.4))
...
const fragment = () => {
  ...
  const ridged = abs(layers).mul(-1).add(1)
  const turbulence = fbmVec3(p.mul(2)).mul(0.3)
  const final = ridged.add(turbulence).clamp(0, 1)
