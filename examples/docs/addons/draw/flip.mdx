...
const fragment = () => {
        const gradient = uv.x
        const wave = sin(uv.y.mul(20).add(iTime.mul(2))).mul(0.5).add(0.5)
        const color = vec3(gradient, wave, gradient.mul(wave))
        const flipped = flip(color, abs(sin(iTime.mul(0.3))))
