...
Quaternions represent rotations in 4D space using unit vectors on the complex hypersphere. This function converts the quaternion representation $q = x\mathbf{i} + y\mathbf{j} + z\mathbf{k} + w$ into its equivalent 3×3 rotation matrix form.
...
<FragmentEditor
        ...
        code={`
const fragment = () => {
        const angle = atan2(uv.y, uv.x)
        const radius = length(uv)
        const q = vec4(
                sin(angle.add(iTime)),
                cos(angle.mul(2)),
                sin(radius.mul(3).add(iTime.mul(0.7))),
                cos(radius.mul(2).sub(iTime.mul(0.3)))
        ).normalize()
