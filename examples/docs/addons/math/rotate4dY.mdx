...
const fragment = () => {
    ...
    const pos = vec3(uv, sin(iTime.mul(0.3)).mul(0.2)).sub(center).mul(6)
    ...
    const angular = atan2(orbital.z, orbital.x).mul(3)
    const probability = exp(radial.negate()).mul(sin(angular).pow(2))
