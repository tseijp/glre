...
const fragment = () => {
        ...
        const color = vec3(
                intensity.mul(sin(pattern.toFloat())),
                intensity.mul(cos(pattern.toFloat().mul(2))),
                intensity.mul(sin(pattern.toFloat().mul(3)))
        )
...
## Crystalline Lattice Formations

Integer modulo operations enable construction of crystalline lattice structures where discrete boundaries create precise geometric formations resembling molecular crystal arrangements.

<FragmentEditor
        isFun
        code={`
const fragment = () => {
        const uv = position.xy.div(iResolution).sub(0.5).mul(8)
        
        // Simple discrete cellular pattern
        const x = uv.x.mul(4).toInt()
        const y = uv.y.mul(4).toInt()
        
        const pattern = modi(x.add(y), int(5))
        const intensity = pattern.toFloat().div(4)
        
        const color = vec3(intensity).mul(vec3(0.8, 0.6, 0.9))
