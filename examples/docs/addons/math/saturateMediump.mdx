...
const fragment = () => {
        ...
        const complex1 = pow(abs(sin(p.x.mul(3).add(t))), 8).mul(10000)
        const complex2 = exp(cos(p.y.mul(2).sub(t.mul(1.3)))).mul(5000)
        const complex3 = pow(length(p).add(sin(t.mul(2))), 6).mul(8000)
