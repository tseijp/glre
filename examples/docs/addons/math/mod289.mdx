...
### Fractal Crystalline Phase Transition Architecture

This revolutionary demonstration creates self-organizing crystalline structures that undergo continuous phase transitions through the mathematical beauty of prime square modular arithmetic. The visualization reveals the hidden geometric relationships encoded within $17^2 = 289$:
...
const fragment = () => {
        ...
        const radius17 = sqrt(baseX.sub(144.5).pow(2).add(baseY.sub(144.5).pow(2)))
        ...
        const orderParameter = symmetryBreak.smoothstep(0.3, 0.7)
