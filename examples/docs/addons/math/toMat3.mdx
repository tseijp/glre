...
const fragment = () => {
        const baseMatrix = mat4(
                cos(iTime), sin(iTime).negate(), 0, 0,
        ...
        const intensity = abs(transformed.z)
        const colorPhase = atan2(transformed.y, transformed.x)
        const color = vec3(
                sin(colorPhase.add(iTime)).mul(0.5).add(0.5),
                sin(colorPhase.add(iTime).add(2.0)).mul(0.5).add(0.5),
                intensity
        ).mul(intensity)
