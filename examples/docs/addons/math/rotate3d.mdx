...
### Dynamic Crystalline Structure Formation

This demonstration explores three-dimensional rotation applied to position vectors, creating dynamic crystal-like formations through systematic axis rotations combined with temporal evolution.
...
const fragment = () => {
    ...
    const pos = vec3(uv, 0).sub(center).mul(3)
    const axis = vec3(1, 1, 0).normalize()
    const angle = iTime.mul(0.5)
    const rotated = rotate3d(axis, angle).mul(pos)
    const pattern = sin(rotated.length().mul(8)).mul(0.5).add(0.5)
...
const fragment = () => {
    const center = vec3(0.5, 0.5, 0)
    const pos = vec3(uv, sin(iTime.mul(0.3))).sub(center).mul(2)
    ...
    const rings = smoothstep(0.1, 0.0, abs(sin(distance.mul(15))))
