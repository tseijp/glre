...
const fragment = () => {
        ...
        const threshold3 = 0.85
        
        const filter1 = highPass(normalized, threshold1)
        const filter2 = highPass(filter1, threshold2.div(1.sub(threshold1)))
        const filter3 = highPass(filter2, threshold3.div(1.sub(threshold2)))
        
        // Create layered color response
        const base = vec3(0.1, 0.0, 0.2)
        const layer1 = vec3(0.8, 0.2, 0.4).mul(filter1)
        const layer2 = vec3(0.4, 0.9, 0.3).mul(filter2)
        const layer3 = vec3(1.0, 0.8, 0.1).mul(filter3)
