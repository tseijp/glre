---
title: 'Adaptive Threshold Filtering'
...
const fragment = () => {
        ...
        const noise = sin(uv.x.mul(15).add(time)).mul(cos(uv.y.mul(12).sub(time.mul(0.7))))
        const blurred = sin(uv.x.mul(3).add(time.mul(0.5))).mul(cos(uv.y.mul(2.5).sub(time.mul(0.3))))
...
const fragment = () => {
        ...
        const detail = sin(uv.x.mul(20).add(time)).add(cos(uv.y.mul(18).sub(time.mul(1.2))))
        const background = sin(uv.x.mul(2).add(time.mul(0.3))).add(cos(uv.y.mul(1.8).sub(time.mul(0.7))))
        ...
        const red = edges.mul(sin(colorShift).mul(0.5).add(0.7))
        const green = edges.mul(cos(colorShift.mul(1.3)).mul(0.5).add(0.6))
        const blue = edges.mul(sin(colorShift.mul(0.7)).mul(0.5).add(0.8))
...
const fragment = () => {
        ...
        const pattern = sin(uv.x.mul(8).add(time)).mul(cos(uv.y.mul(6).sub(time.mul(0.8))))
        const smooth1 = sin(uv.x.mul(2).add(time.mul(0.4))).mul(cos(uv.y.mul(1.5).sub(time.mul(0.5))))
        const smooth2 = sin(uv.x.mul(4).add(time.mul(0.7))).mul(cos(uv.y.mul(3).sub(time.mul(0.9))))
        ...
        const timeColor = sin(time.mul(3)).mul(0.3).add(0.7)
        const color = vec3(combined.mul(timeColor), combined.mul(sin(time.mul(2.3)).mul(0.3).add(0.6)), combined.mul(cos(time.mul(1.7)).mul(0.3).add(0.8)))
...
Adaptive thresholding implements local statistical analysis through spatial comparison operations. The algorithm evaluates each pixel against its neighborhood characteristics, producing results that maintain detail in varying illumination conditions.
