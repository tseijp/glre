---
title: 'Low-Discrepancy Sampling for Monte Carlo Rendering'
...
const fragment = () => {
        ...
        const index = coord.x.add(coord.y.mul(64.0)).toUint()
        
        const sample = hammersley(index, 64)
...
const fragment = () => {
        ...
        const sampleIndex = pos.x.add(pos.y.mul(32.0)).add(timeShift).toUint()
...
const fragment = () => {
        ...
        const hammersleySample = hammersley(cellIndex.toUint(), sampleCount)
