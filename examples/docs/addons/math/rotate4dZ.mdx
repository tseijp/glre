...
const fragment = () => {
    ...
    const fieldAngle = atan2(pos.y, pos.x).mul(4).add(iTime.mul(3))
    const rotation = rotate4dZ(fieldAngle)
    const confined = rotation.mul(vec4(pos, 1))
    const plasma = exp(abs(toroidal).negate().mul(8))
    const temperature = sin(confined.x.mul(6)).mul(cos(confined.y.mul(6)))
