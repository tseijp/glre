---
title: 'Anti-aliased Floor Function'
...
const fragment = () => {
        ...
        const color = pattern.mix(vec3(0.2, 0.3, 0.8), vec3(0.9, 0.7, 0.1))
        return vec4(color, 1)
}`}
/>
...
### Pixel-perfect Animation Flow

Moving patterns with traditional floor create stuttering motion artifacts. Anti-aliased floor ensures smooth animation flow regardless of movement speed or direction.

<FragmentEditor
        isFun
        code={`
const fragment = () => {
        const wave = sin(uv.x.mul(10).add(iTime.mul(3))).mul(2)
        const tiles = aafloor(uv.y.mul(15).add(wave))
        const gradient = tiles.div(15).saturate()
        const shimmer = sin(tiles.add(iTime.mul(4))).mul(0.3).add(0.7)
        const color = gradient.mul(shimmer).mul(vec3(0.6, 0.9, 0.4))
        return vec4(color, 1)
}`}
/>
