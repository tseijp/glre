...
$$
\text{aafloor}(x) = \begin{cases}
x - \text{fract}(x) & \text{if } \text{fract}(x) < 1 - \text{afwidth} \\
\text{map}(\text{fract}(x), 1-\text{afwidth}, 1, x-\text{fract}(x), x) & \text{otherwise}
\end{cases}
$$

Where $\text{fract}(x) = x - \lfloor x \rfloor$ represents the fractional part.
