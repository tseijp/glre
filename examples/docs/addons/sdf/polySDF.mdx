...
const fragment = () => Scope(() => {
        ...
        Loop(4, ({ i }) => {
                const sides = i.add(3)
                const offset = vec2(
                        i.mod(2).toFloat().mul(0.4).add(0.2),
                        i.div(2).floor().toFloat().mul(0.4).add(0.2)
                )
                const p = uv.sub(offset).mul(3)
                const d = polySDF(p.add(0.5), sides).sub(0.2).abs().sub(0.02)
                minDist.assign(minDist.min(d))
        })
        const brightness = step(minDist, 0).mul(0.9).add(smoothstep(0.02, 0, minDist.abs()).mul(0.3))
