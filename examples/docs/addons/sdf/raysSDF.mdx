...
code={`
const fragment = () => {
        const rays8 = raysSDF(uv, 8)
        const pattern = step(0.6, rays8)
        const gradient = rays8.mul(0.8).add(0.2)
        const color = pattern.mul(gradient)
...
const fragment = () => Scope(() => {
        ...
        Loop(4, ({ i }) => {
                ...
                const rayPattern = raysSDF(uv, rayCount)
        ...
        const hue = raysSDF(uv, 16).mul(0.3).add(0.1)
