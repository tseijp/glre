...
const fragment = () => Scope(() => {
        ...
        Loop(6, ({ i }) => {
                const angle = i.toFloat().mul(1.047)
                const radius = 0.3
                const a = vec2(sin(angle), cos(angle)).mul(radius).add(0.5)
                const b = vec2(sin(angle.add(3.14)), cos(angle.add(3.14))).mul(radius.mul(0.5)).add(0.5)
                const dist = lineSDF2D(uv, a, b)
                const line = smoothstep(0.015, 0.008, dist)
                brightness.assign(brightness.max(line))
        })
        return vec4(brightness.mul(vec3(0.3, 0.9, 0.6)), 1)
})`}
/>