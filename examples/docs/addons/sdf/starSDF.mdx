...
const fragment = () => {
        ...
        const vertices = iTime.mul(0.3).add(3).floor().toInt()
        const scale = sin(iTime.mul(2)).mul(0.3).add(0.4)
        const dist = starSDF(coord.mul(0.5).add(0.5), vertices, scale)
        const brightness = smoothstep(0.02, 0, dist).mul(0.9)
