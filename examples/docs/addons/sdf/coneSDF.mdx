...
const fragment = () => {
        ...
        const coneParam = vec3(0.7, 1.2, 0.8)
        const dist = coneSDF(pos, coneParam)
        const inside = step(dist, 0).mul(0.9)
        const edge = smoothstep(0.03, 0, dist.abs()).mul(0.5)
        return vec4(inside.add(edge).mul(vec3(1, 0.6, 0.2)), 1)
}`}
/>
