...
const fragment = () => {
        ...
        const march = Fn(([eye, dir]: [Vec3, Vec3]) => {
                const p = eye.toVar()
                const d2d = circleSDF(p, vec2(0.8)).min(
                        rectSDF(vec2(p.x.add(0.3), p.y), vec2(0.3, 0.6))
                ).max(circleSDF(vec2(p.x.sub(0.2), p.y.add(0.1)), vec2(0.25)).negate())
                const d = opExtrude(p, d2d, ...args).toVar()
                Loop(16, ({ i }) => {
                        If(d.lessThanEqual(eps.x), () => {
                                const dx2d = circleSDF(vec2(p.add(eps.xyy).x, p.add(eps.xyy).y), vec2(0.8)).min(
                                               rectSDF(vec2(p.add(eps.xyy).x.add(0.3), p.add(eps.xyy).y), vec2(0.3, 0.6))
                                       ).max(circleSDF(vec2(p.add(eps.xyy).x.sub(0.2), p.add(eps.xyy).y.add(0.1)), vec2(0.25)).negate())
                                const dy2d = circleSDF(vec2(p.add(eps.yxy).x, p.add(eps.yxy).y), vec2(0.8)).min(
                                               rectSDF(vec2(p.add(eps.yxy).x.add(0.3), p.add(eps.yxy).y), vec2(0.3, 0.6))
                                       ).max(circleSDF(vec2(p.add(eps.yxy).x.sub(0.2), p.add(eps.yxy).y.add(0.1)), vec2(0.25)).negate())
                                const dz2d = circleSDF(vec2(p.add(eps.yyx).x, p.add(eps.yyx).y), vec2(0.8)).min(
                                               rectSDF(vec2(p.add(eps.yyx).x.add(0.3), p.add(eps.yyx).y), vec2(0.3, 0.6))
                                       ).max(circleSDF(vec2(p.add(eps.yyx).x.sub(0.2), p.add(eps.yyx).y.add(0.1)), vec2(0.25)).negate())
                        ...
                        const d2dNew = circleSDF(p, vec2(0.8)).min(
                                rectSDF(vec2(p.x.add(0.3), p.y), vec2(0.3, 0.6))
                        ).max(circleSDF(vec2(p.x.sub(0.2), p.y.add(0.1)), vec2(0.25)).negate())
