...
const fragment = () => {
        ...
        const minBounds = vec3(sin(t).mul(1.8).sub(0.3), cos(t.mul(1.3)).mul(0.7).sub(0.2), 0)
        const maxBounds = vec3(sin(t.mul(0.7)).mul(0.9).add(0.6), cos(t.mul(0.9)).mul(1.5).add(0.8), 0)
        const originalBox = AABB({ minBounds, maxBounds })
        const squaredBox = square(originalBox)
        const pos = vec3(p, 0)
        const inOriginal = contain(originalBox, pos)
        const inSquared = contain(squaredBox, pos)
        ...
        const sqEdge = min(
                min(abs(p.x.sub(squaredBox.minBounds.x)), abs(p.x.sub(squaredBox.maxBounds.x))),
                min(abs(p.y.sub(squaredBox.minBounds.y)), abs(p.y.sub(squaredBox.maxBounds.y)))
        )
