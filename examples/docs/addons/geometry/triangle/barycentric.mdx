...
# triangleBarycentric: Barycentric Coordinate Computation
...
**`triangleBarycentric(a, b, c)`** computes normalized barycentric coordinates directly from three vertices. The computation projects vectors onto a coordinate system defined by the triangle edges.

**`triangleBarycentricFromPosition(a, b, c, pos)`** determines barycentric coordinates of a specific position relative to the triangle using cross product area ratios:
...
const fragment = () => {
	...
	const v1 = vec3(-0.8, -0.6, 0)
	const v2 = vec3(0.8, -0.6, 0)
	const v3 = vec3(0, 0.8, 0)
	const pos = vec3(p, 0)
	const bary = barycentricFromPosition(v1, v2, v3, pos)
	return vec4(abs(bary), 1)
}`}
/>
