...
<FragmentEditor
        ...
        code={`
const fragment = () => {
        const bands = floor(uv.x.mul(16))
        const gamma_input = bands.div(16)
        const linear_output = gamma2linear(gamma_input)
        const comparison = select(gamma_input, linear_output, uv.y.greaterThan(0.5))
...
const fragment = () => {
        ...
        const blend_factor = sin(iTime.mul(2)).mul(0.5).add(0.5)
