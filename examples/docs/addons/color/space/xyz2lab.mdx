...
const fragment = () => {
        ...
        const normalizedL = labColor.x.div(100)
        const normalizedA = labColor.y.add(127).div(254)
        const normalizedB = labColor.z.add(127).div(254)
        
        return vec4(normalizedL, normalizedA, normalizedB, 1)
}`}
/>

### LAB Space Chromatic Expansion

<FragmentEditor
        isFun
        code={`
const fragment = () => {
        const centerXYZ = vec3(50, 50, 50)
        const offset = vec2(uv.sub(0.5).mul(100))
        
        const xyzColor = centerXYZ.add(vec3(offset.x, offset.y, 0))
        const labColor = xyz2lab(xyzColor.max(0))
        
        const chromaA = labColor.y.div(127)
        const chromaB = labColor.z.div(127)
        const lightness = labColor.x.div(100)
        
        return vec4(chromaA.abs(), chromaB.abs(), lightness, 1)
}`}
/>