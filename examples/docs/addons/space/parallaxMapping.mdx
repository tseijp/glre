...
The system uses configurable constants for precision control:

- **PARALLAXMAPPING_SCALE**: Displacement magnitude (0.01)
- **PARALLAXMAPPING_NUMSEARCHES**: Base layer count (10.0)
...
const fragment = () => {
        ...
        const viewDir = vec3(angle.sin(), angle.cos(), 2).normalize()
        const relief = parallaxMappingRelief(heightTex, viewDir, uv)
        const occlusion = parallaxMappingOcclusion(heightTex, viewDir, uv)
        const blend = uv.x.step(0.5)
        const final = relief.mix(occlusion, blend)
        const depth = final.sub(uv).length().mul(50)
