...
const fragment = () => {
        const scale = 6
        const tile = triTileScaled(uv, scale)
        const dist = tile.xy.length()
        const pattern = smoothstep(0.2, 0.4, dist).sub(smoothstep(0.6, 0.8, dist))
