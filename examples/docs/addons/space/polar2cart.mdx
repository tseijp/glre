...
<FragmentEditor
        ...
        code={`
const fragment = () => {
        const angle = uv.x.mul(6.28)
        const radius = uv.y.mul(0.4)
        const reconstructed = polar2cart(vec2(angle, radius))
        const pattern = reconstructed.x.mul(reconstructed.y).mul(50).sin().abs()
