...
X-axis rotations create pitch motion in 3D space, commonly used for:

- Camera pitch control
- Object tilting animations
- 3D model orientation adjustments
...
const fragment = () => {
        ...
        const angle = sin(iTime).mul(1.5)
        const pos = vec3(uv.x, uv.y, 0.3).sub(0.5).mul(3)
        const rotated = rotateXVec3Center(pos, angle, center)
        const depth = rotated.z.add(1).mul(0.5)
        const color = vec3(depth, rotated.y.add(0.5), rotated.x.add(0.5))
        return vec4(color, 1)
}`}
/>
