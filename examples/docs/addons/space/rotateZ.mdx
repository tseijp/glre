...
Z-axis rotations create roll motion in 3D space, commonly used for:

- 2D coordinate transformations
- Screen-space effects
- Object spinning animations in the XY-plane
...
<FragmentEditor
        ...
        code={`
const fragment = () => {
        const center = vec3(0.2, 0.3, 0)
        const angle = sin(iTime.mul(0.8)).mul(3.14)
        const pos = vec3(uv.sub(0.5).mul(1.5), 0)
        const rotated = rotateZVec3Center(pos, angle, center)
        const dist = length(rotated.xy.sub(center.xy))
        const color = vec3(1).sub(vec3(dist))
        return vec4(color, 1)
}`}
/>
