...
const fragment = () => {
        ...
        const far = float(50)
        const linear = linearizeDepth(depth, near, far)
        const color = linear.div(far.sub(near)).mul(2)
        return vec4(color, color.mul(0.5), float(1).sub(color), 1)
}`}
/>

## Depth Distribution Properties

**Perspective Non-linearity**: Standard depth buffers allocate more precision to near objects and less to distant ones.

**Linear Interpolation**: Linearized depth provides uniform precision distribution across the depth range.

**Distance Calculations**: Linear depth enables accurate distance-based effects like fog, depth of field, and volumetric lighting.
