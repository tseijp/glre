...
const fragment = () => {
        const lissajous = Fn(([grid, xy]) => {
                ...
                Loop(int(32), ({ i }) => {
                        const t = float(i)
                        const pos = iTime.add(t).mul(grid).mul(7).sin()
        ...
        const generateGrid = Fn(([sample]) => {
                const ret = vec3(0).toVar()
                Loop(int(sample), ({ i }) => {
                        Loop(int(sample), ({ i: j }) => {
                                const grid = vec2(i, j).add(1)
                                const center = grid.sub(sample.div(2).add(0.5)).mul(2)
                                const pos = uv.sub(0.5).mul(sample).mul(2).sub(center)
                                const pattern = lissajous(grid, pos)
                                const color = vec3(grid.div(sample), 0.5).mul(pattern)
                                ret.assign(ret.add(color))
                        })
                })
                return ret
        })
        return vec4(generateGrid(5), 1)
}`}
/>
...
const fragment = () => {
        ...
        const red = hue.sin()
        const green = hue.add(29).sin()
        const blue = hue.add(4.18).sin()
        const color = vec3(red, green, blue).mul(pattern)
        return vec4(color, 1)
}`}
/>
...
const fragment = () => {
        ...
        const distances = vec3(
                vec2(pointsX.x, pointsY.x).sub(pos).length(),
                vec2(pointsX.y, pointsY.y).sub(pos).length(),
                vec2(pointsX.z, pointsY.z).sub(pos).length()
        )
...
const fragment = () => {
        ...
        const pattern = generatePattern(pos)
