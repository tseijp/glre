...
<FragmentEditor
        ...
        code={`
const fragment = () => {
        // Without correction, circles become ellipses
        const wrongCircle = step(length(uv.sub(0.5)), 0.5)
        // With aspect ratio correction
        const aspect = iResolution.x.div(iResolution.y)
        const correctedPos = vec2(uv.x.sub(0.5).mul(aspect), uv.y.sub(0.5))
        const correctCircle = step(length(correctedPos), 0.5)
        // Show both: left side wrong, right side correct
        const showCorrect = step(0.5, uv.x)
        const circle = mix(wrongCircle, correctCircle, showCorrect)
