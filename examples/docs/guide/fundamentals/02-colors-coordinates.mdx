...
const fragment = () => {
        ...
        const rgb2hsv = Fn(([h, s, v]) => {
                const c = v.mul(s).toVar()
                const x = mod(h.mul(6), 2).sub(1).abs().oneMinus().mul(c)
                const i = h.mul(6).floor()
                const ret = Fn(([c, x, i]) => {
                        If(i.equal(0), () => vec3(c, x, 0))
                        .ElseIf(i.equal(1), () => vec3(x, c, 0))
                        .ElseIf(i.equal(2), () => vec3(0, c, x))
                        .ElseIf(i.equal(3), () => vec3(0, x, c))
                        .ElseIf(i.equal(4), () => vec3(x, 0, c))
                        .Else(() => vec3(c, 0, x))
                })
                const m = v.sub(c)
                return ret(c, x, i).add(m)
        })
