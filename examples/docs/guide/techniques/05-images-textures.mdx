...
const fragment = () => {
        ...
        const checker = mod(grid.x.add(grid.y), 2.0)
        // Simple procedural overlay using vec
        const noiseUV = uv.mul(20).sin()
        const noise = noiseUV.x.mul(noiseUV.y).mul(0.5).add(0.5)
        const overlayRGB = vec3(noise).mul(vec3(0.8, 0.6, 1.0)).add(vec3(0.2, 0.4, 0.0))
        const overlayColor = vec4(overlayRGB, 1.0)
        // Blend with texture
