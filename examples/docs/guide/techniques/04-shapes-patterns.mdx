...
const fragment = () => {
        const msaa = Fn(([AA]) => {
                ...
                Loop(int(AA), ({ i: m }) => {
                        Loop(int(AA), ({ i: n }) => {
                                const offset = vec2(m, n)
                                const diff = offset.div(AA).sub(0.5).div(iResolution)
                                const distance = pos.add(diff).length()
                                const circle = step(distance, 0.3)
                                const newRet = ret.add(circle)
                                ret.assign(newRet)
                        })
                })
        ...
        const msaaResult = msaa(4)
        return vec4(vec3(msaaResult), 1)
}`}
/>
...
const fragment = () => {
        ...
        const pos1 = vec2(-gap, gap).add(pos)
        const pos2 = vec2(gap, gap).add(pos)
        const pos3 = vec2(-gap, -gap).add(pos)
        const pos4 = vec2(gap, -gap).add(pos)
        const shapes = polygon(pos1, 3, 0.1)
                  .add(polygon(pos2, 4, 0.15))
                  .add(polygon(pos3, 6, 0.15))
                  .add(polygon(pos4, 8, 0.15))
...
const fragment = () => {
        ...
        // Create star
        const starDist = sdPentagram(pos, 0.2)
...
const fragment = () => {
        ...
        const sineWave = pos.x.mul(4).sin().mul(0.3)
        const sineDistance = pos.y.sub(sineWave).abs()
        const sineLine = smoothstep(0.05, 0, sineDistance)
        // Parabola
        const parabola = pow(pos.x, 3)
        const parabolaDistance = pos.y.sub(parabola).abs()
        const parabolaLine = smoothstep(0.05, 0, parabolaDistance)
        // Circle outline
        const circleDistance = pos.length().sub(0.5).abs()
...
const fragment = () => {
        ...
        const generateFractal = Fn(([uv, iterations]) => {
                ...
                Loop(int(iterations), ({ i }) => {
                        ...
                        const layerRatio = layerIndex.div(float(iterations))
                        const weight = layerRatio.oneMinus()
                        const circleVec = vec3(circle).mul(weight)
                        color.assign(color.add(circleVec))
                })
                const iterationsFloat = float(iterations)
                return color.div(iterationsFloat) // Normalize
        })
...
const fragment = () => {
        ...
        const t = iTime.sin()
        // Pattern A: Circles
        const circles = uv.sub(0.5).length().mul(10).sin()
        // Pattern B: Stripes
        const stripes = uv.x.mul(10).sin()
        // Pattern C: Grid
        const gridX = uv.x.mul(10).sin()
        const gridY = uv.y.mul(10).sin()
        const grid = gridX.mul(gridY)
