        ...
        const msaa = Fn(([AA]) => {
                ...
                Loop(int(AA), ({ i: m }) => {
                        Loop(int(AA), ({ i: n }) => {
                                ...
                                const diff = offset.div(AA).sub(0.5).div(AA)
...
const fragment = () => {
        ...
        // Parabola
        const parabola = pos.x.mul(pos.x).mul(2)
...
const fragment = () => {
        ...
        const voronoiPattern = Fn(([gridRow, gridCol]) => {
                ...
                Loop(int(9), ({ i }) => {
                        ...
                        const point = random2(gridRow.add(offset))
                        const dist = length(offset.add(point).sub(gridCol))
...
const fragment = () => {
        ...
        const color = length(local.sub(0.5))
