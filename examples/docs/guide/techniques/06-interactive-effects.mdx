...
const fragment = () => {
        const pos = position
        const clicking = uniform(float(), 'isClicking')
        const distance = length(pos.xy.sub(iMouse))
        // クリック時に効果を強化
        const baseIntensity = oneMinus(distance)
        const clickMultiplier = clicking.mul(2).add(1)
...
const fragment = () => {
        const pos = position
        const dragVector = uniform(float(), 'dragVector')
        // ドラッグベクトルによる歪み
        const distortion = dragVector.mul(0.5)
        const distortedPos = pos.xy.add(distortion)
        // 歪んだ座標でのパターン
        const pattern = sin(distortedPos.x.mul(8)).mul(sin(distortedPos.y.mul(8)))
...
const fragment = () => {
        const pos = position
        const colorMode = uniform(float(), 'colorMode')
        // キー入力に応じた色の変更
        ...
        // パターンの描画
        const pattern = sin(pos.x.mul(10)).mul(sin(pos.y.mul(10)))
        const finalColor = color.mul(pattern)
...
const fragment = () => {
        const time = iTime
        const pos = position
        const freq = uniform(float(), 'frequency')
        const amp = uniform(float(), 'amplitude')
        const speed = uniform(float(), 'speed')
        // パラメータを使った波のアニメーション
...
document.addEventListener('keydown', (event) => {
        switch (event.code) {
                case 'ArrowUp':
                        parameters.frequency = Math.min(parameters.frequency + 0.5, 20)
                        break
                case 'ArrowDown':
                        parameters.frequency = Math.max(parameters.frequency - 0.5, 1)
                        break
                case 'ArrowLeft':
                        parameters.speed = Math.max(parameters.speed - 0.1, 0.1)
                        break
                case 'ArrowRight':
                        parameters.speed = Math.min(parameters.speed + 0.1, 5)
                        break
                case 'Space':
                        parameters.amplitude = parameters.amplitude === 0.5 ? 1 : 0.5
                        break
        }
...
const fragment = () => {
        const pos = position
        const touch1 = uniform(float(), 'touch1')
        const touch2 = uniform(float(), 'touch2')
        const touchCount = uniform(float(), 'touchCount')
        let color = vec3(0.1, 0.1, 0.2) // 背景色
        // 第1タッチポイント
        If(touchCount.greaterThan(0.5), () => {
                const distance1 = length(pos.xy.sub(touch1))
                const effect1 = smoothstep(0.5, 0, distance1)
        ...
        If(touchCount.greaterThan(1.5), () => {
                ...
                const effect2 = smoothstep(0.5, 0, distance2)
...
const fragment = () => {
        const pos = position
        const scale = uniform(float(), 'pinchScale')
        const center = uniform(float(), 'pinchCenter')
        // ピンチによるズーム効果
...
const fragment = () => {
        const pos = position
        const accel = uniform(float(), 'acceleration')
        // 重力方向による効果
...
const fragment = () => {
        const pos = position
        const rotation = uniform(float(), 'deviceRotation')
        // デバイスの回転に応じた座標変換
        const angle = rotation.z.mul(01) // 角度を調整
        const cosA = cos(angle)
        const sinA = sin(angle)
        const rotatedPos = vec2(pos.x.mul(cosA).sub(pos.y.mul(sinA)), pos.x.mul(sinA).add(pos.y.mul(cosA)))
        // 回転した座標でのパターン
        const pattern = sin(rotatedPos.x.mul(10)).mul(sin(rotatedPos.y.mul(10)))
        const color = vec3(pattern)
        return vec4(color, 1)
}`}
/>
...
const fragment = () => {
        const pos = position
        const particles = uniform(float(), 'particlePositions')
        ...
        Loop(25, ({ i }) => {
                ...
                const particle = smoothstep(0.05, 0, distance)
