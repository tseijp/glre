...
const thickness = constant(float(0.01))
...
const harnack = Fn(([ro, rd]) => {
        ...
        const ray = gyroid(pos).toVar('ray')
        const eps = epsilon.mul(ray.n.length()).toVar('eps')
        If(
                levelset
                        .sub(thickness)
                        .sub(ray.t)
                        .max(ray.t.sub(levelset.add(thickness)))
                        .lessThan(eps),
                () => {
                        return ray
                }
        ...
        Loop(iterations, () => {
                ...
                ray.assign(gyroid(pos))
                const R = sphereRadius.add(0.5).sub(pos.length()).toVar('R')
                const shift = sqrt(2).mul(R).exp().mul(unit_shift).toVar('shift')
                const r = getMaxStep4D(ray.t, R, levelset, shift).toVar('r')
                If(r.greaterThanEqual(overstep).and(closeToLevelset(ray.t, levelset, epsilon, ray.n.length())), () => {
                        return ray
                })
