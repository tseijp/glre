        ...
        fragment: () => {
                ...
                const blurFilter = Fn(([uv, blurRadius, size]) => {
                        ...
                        Loop(totalSamples, ({ i }) => {
                                ...
                                // UV座標をクランプしてアーティファクトを防ぐ
                                const clampedUV = clamp(sampleUV, vec2(0), vec2(1))
                                const sampleColor = texture(uniform('mainTexture'), clampedUV)
...
画像とテクスチャの操作をマスターしたら、次は[インタラクティブな表現](06-interactive-effects.mdx)で、ユーザー操作に反応するエフェクトを学びましょう。
