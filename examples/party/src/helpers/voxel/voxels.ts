...
const generateRegionId = (config: RegionConfig): string => {
...
const parseFromTiles = async (glbBuffer: ArrayBuffer): Promise<any> => {
        if (!glbBuffer || glbBuffer.byteLength === 0) return { tris: [], materials: [], textures: [], aabb: { min: [0, 0, 0], max: [32, 16, 32] }, model: { extent: [32, 16, 32], center: [16, 8, 16] } }
        return await loader(glbBuffer)
}
