...
export const toRGBA = (img: any): Tex | undefined => {
        if (!img) return
        if (img.data && img.width && img.height) {
                const w = img.width | 0
                const h = img.height | 0
                const c = (img.components || 4) | 0
                const src: Uint8Array = img.data
                if (c === 4) return { w, h, dat: src }
                if (c === 3) {
                        const dst = new Uint8Array(w * h * 4)
                        for (let i = 0, si = 0, di = 0; i < w * h; i++) {
                                dst[di++] = src[si++]
                                dst[di++] = src[si++]
                                dst[di++] = src[si++]
                                dst[di++] = 255
...
export const buildFromGLTF = (json: any) => {
        const textures: Tex[] = []
        const map = new Map<GLTFImagePostprocessed, number>()
        const imgs = json.images || []
        for (let i = 0; i < imgs.length; i++) {
                const t = toRGBA(imgs[i]?.image)
                if (t) {
                        map.set(imgs[i], textures.length)
                        textures.push(t)
                } else map.set(imgs[i], -1)
        }
        const materials: Mat[] = (json.materials || []).map((m: any) => {
                ...
                if (bct && bct.texture && bct.texture.source) {
                        const idx = map.get(bct.texture.source)
        ...
        for (const mesh of json.meshes || [])
                for (const prim of mesh.primitives || []) {
                        ...
                        if (!pa) continue
                        const pos = pa.value as Float32Array
                        const ua = prim.attributes?.TEXCOORD_0
                        const uv = ua ? (ua.value as Float32Array) : new Float32Array(0)
                        const ia = prim.indices
                        const ind = ia ? ia.value : null
                        const mi = prim.material ? (json.materials || []).indexOf(prim.material) : 0
                        ...
                        if (ind) {
                                for (let t = 0; t < ind.length; t += 3) push(ind[t + 0] as number, ind[t + 1] as number, ind[t + 2] as number)
                        } else {
                                for (let i = 0; i < pos.length / 3; i += 3) push(i + 0, i + 1, i + 2)
                        }
                }
