...
export const createTileTraversal = (tileset: Tileset): TileTraversal => {
        ...
        const update = async (viewport: Viewport): Promise<Tile[]> => {
                initTileset3D()
                if (!tileset3D) return []
                
                const frustum = createFrustumFromViewport(viewport)
                const frameState = {
                        camera: {
                                position: [viewport.longitude, viewport.latitude, 1000],
                                direction: [0, 0, -1],
                                up: [0, 1, 0]
                        },
                        viewport: frustum,
                ...
                await tileset3D.selectTiles(frameState)
...
export const createFrustumFromViewport = (viewport: Viewport) => {
        ...
        const position = [
                longitude + Math.sin(bearing) * zoom * 0.01,
                latitude + Math.cos(bearing) * zoom * 0.01,
                1000 / zoom
        ]
...
const isTileInBounds = (tile: Tile, bounds: { min: number[], max: number[] }): boolean => {
        ...
        if (bv.box) {
                ...
                return center[0] >= bounds.min[0] && center[0] <= bounds.max[0] &&
                       center[1] >= bounds.min[1] && center[1] <= bounds.max[1] &&
                       center[2] >= bounds.min[2] && center[2] <= bounds.max[2]
        }
        
        if (bv.sphere) {
                const center = bv.sphere.slice(0, 3)
                const radius = bv.sphere[3]
                return center[0] - radius >= bounds.min[0] && center[0] + radius <= bounds.max[0] &&
                       center[1] - radius >= bounds.min[1] && center[1] + radius <= bounds.max[1] &&
                       center[2] - radius >= bounds.min[2] && center[2] + radius <= bounds.max[2]
        }
