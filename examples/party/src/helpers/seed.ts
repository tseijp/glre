...
export const seedTraditionalColors = async (DB: D1Database) => {
        ...
        for (const color of TRADITIONAL_COLORS_DATA) {
                try {
                        await db.insert(traditionalColors).values({
        ...
        const culturalLessons = [
                {
                        ...
                        learningObjectives: JSON.stringify([
                                'Recognize 10 traditional Japanese color names',
                                'Understand seasonal color associations',
                                'Apply cultural context to color choices'
                        ]),
                        creatorId: 'system'
                },
                {
                        ...
                        learningObjectives: JSON.stringify([
                                'Understand the Five Elements theory',
                                'Apply elemental color harmony rules',
                                'Create balanced color compositions'
                        ]),
                        creatorId: 'system'
                },
                {
                        ...
                        learningObjectives: JSON.stringify([
                                'Implement seasonal color transitions',
                                'Maintain cultural authenticity',
                                'Create living digital environments'
                        ]),
                        creatorId: 'system'
                }
        ]
...
export const seedAllCulturalData = async (DB: D1Database) => {
        const results = await Promise.all([
                seedTraditionalColors(DB),
                seedEducationalContent(DB),
                seedCulturalEvents(DB)
        ])
