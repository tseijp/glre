...
GLRE の中核機能は、CPU 側のデータを GPU 側のシェーダプログラムに送信するための設定を自動的に構築することである
...
複雑な型が必要な場合は一旦 any を使用し、as キーワードを活用して型の橋渡しを行う
...
厳密な型定義よりも実装の簡潔性を優先し、必要に応じて as キーワードで型を明示する
複雑な型推論が必要な場合は、一時的に any を使用して実装を進め、後から段階的に型を詳細化する
...
GLRE ノードシステムは、JavaScript から GLSL/WGSL シェーダコードを生成するドメイン固有言語である
...
| ---------------------- | ---- | ---------- | -------- |
| `.equal(n)`            | `==` | 等価比較   | `bool`   |
| `.notEqual(n)`         | `!=` | 不等価比較 | `bool`   |
| `.lessThan(n)`         | `<`  | 小なり比較 | `bool`   |
| `.greaterThan(n)`      | `>`  | 大なり比較 | `bool`   |
| `.lessThanEqual(n)`    | `<=` | 以下比較   | `bool`   |
| `.greaterThanEqual(n)` | `>=` | 以上比較   | `bool`   |
...
| `.bitXor(n)`     | `^`  | ビット排他和 | 入力型   |
| `.bitNot()`      | `~`  | ビット否定   | 入力型   |
| `.shiftLeft(n)`  | `<<` | 左シフト     | 入力型   |
| `.shiftRight(n)` | `>>` | 右シフト     | 入力型   |
